...
import { signUpInput } from "@manishnangliya/medium-common-validation";
import { BACKEND_URL } from "../config";
import { UserContext } from "../store/Context";

export function SignupAuth() {
    const userContext = useContext(UserContext);
    const [inputPosts, setInputPosts] = useState<signUpInput>({
        name: "",
        email: "",
        password: ""
    })
    const navigate = useNavigate();
    function nameHandle(e: ChangeEvent<HTMLInputElement>) {
        setInputPosts({
            ...inputPosts,
            name: e.target.value
        })
    ...
    return <div className="h-screen flex flex-col justify-center">
        <div className="flex flex-col items-center gap-3 ">
            ...
            <TextBox label="Name" placeholder="Enter your name" onChange={nameHandle} type="text" />
            ...
            <CustomButton onClick={signUpHandle} label="Sign Up" />
        </div>
    </div>
}